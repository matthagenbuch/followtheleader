...
func NewBackend(c map[string]string, logger *log.Logger) (physical.Backend, error) {
	logger.Println("configuring backend")
	...
	if haEnabled {
		logger.Println("creating HA client")
		var err error
		ctx := context.Background()
		haClient, err = spanner.NewClient(ctx, database,
			// TODO read app name and version from metadata?
			option.WithUserAgent("followtheleader/0.0.1"),
		)
		if err != nil {
